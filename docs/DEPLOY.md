...
Тестовый вход по имени **включён по умолчанию** (без ВК): каждый тестер получает свой
аккаунт, чаты и сделки работают. **При запуске VK ID добавь в `.env`: `ALLOW_TEST_LOGIN=0`**
и пересобери — тестовый вход закроется (403).
