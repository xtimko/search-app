...
## 7. VK ID OAuth («Войти через ВК» на сайте)
1. https://dev.vk.com → приложение (Web/сайт; Mini App тоже подойдёт, если в нём доступен VK ID).
2. В настройках VK ID / OAuth укажи:
   - **Базовый домен**: `search-app.ru`
   - **Доверенный redirect URL**: `https://search-app.ru/api/auth/vk/callback`
3. В `.env` на сервере добавь: `VK_APP_ID` (ID приложения), `VK_APP_SECRET` (защищённый ключ),
   `SESSION_SECRET` (сгенерировать: `openssl rand -hex 32`). Пересобери: `sudo docker compose up -d --build`.
4. Проверка: кнопка «Войти через VK» в топбаре → окно VK ID → возврат на сайт с именем и аватаром из ВК.

> В проде разделы «Мой сток», «Профиль», «Чаты» без входа недоступны (401 + гейт входа).
> Локально (не production) без входа работает dev-продавец.

## 7а. Тестовый период (пока VK ID не подключён)
В `.env` добавь `ALLOW_TEST_LOGIN=1` и пересобери — на сайте заработает «тестовый вход»
по имени (без ВК): каждый тестер получает свой аккаунт, чаты и сделки работают.
**При запуске VK ID удали `ALLOW_TEST_LOGIN` из `.env`** (тестовый вход закроется, 403).
